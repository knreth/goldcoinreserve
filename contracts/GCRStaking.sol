...
contract GCRStaking is Ownable {
    ...
    constructor(address _gcrToken, address _rewardsFrom) {
        ...
        require(
            _rewardsFrom != address(0),
            "GCRStaking: invalid reward account"
        );
        gcrToken = _gcrToken;
     ...
     */
    function enterStaking(uint256 _stakeAmount, string calldata _plan)
        external
    {
        bool status;
        uint256 i;
        ...
        require(
            ...
            "GCRStaking: amount is below the minimum allowed"
        );
        ...
        require(
            ...
            "GCRStaking: reached maximum stakes per user"
        );
        ...
        status = IERC20(gcrToken).transferFrom(
            msg.sender,
            address(this),
            _stakeAmount
        );
        ...
        if (rewardAmount > 0) {
            ...
            require(status, "GCRStaking: failed to transfer reward");
        }
    }
    ...
    function withdrawStaking(uint256 _stakeIndex) external {
        ...
        require(
            ...
            "GCRStaking: invalid stake index"
        );
        ...
        require(vestingAmount > 0, "GCRStaking: plan not yet vested");
    ...
    function transferExcessReward(address _to) external onlyOwner {
        ...
        if (excessAmount > 0) {
            excessAmount =
                excessAmount -
                (totalStakedAmount + totalPendingRewardAmount);
    ...
    function _calculateVestingAmount(Stake storage stake, Plan storage plan)
        internal
        returns (uint256, uint256)
    {
        ...
        if (
            ...
            ((stake.stakedAmount - temp) < stake.vestingAmount)
        ) temp = stake.stakedAmount;

        if (
            (temp1 > stake.rewardAmount) ||
            ((stake.rewardAmount - temp1) < stake.vestingReward)
        ) temp1 = stake.rewardAmount;
    ...
    function createPlan(
        ...
        require(
            _vestingPeriod <= _duration,
            "GCRStaking: invalid versting period"
        );
    ...
    function disablePlan(string calldata _name) external onlyOwner {
        require(
            plans[_name].state == 1,
            "GCRStaking: invalid plan or already disabled"
        );
